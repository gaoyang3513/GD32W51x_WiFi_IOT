...
static const struct mpc_sie_memory_range_t MPC_BRAM_RANGE_S = {
...
static const struct mpc_sie_memory_range_t MPC_BRAM_RANGE_NS = {
...
static int32_t BRAM_MPC_GetBlockSize(uint32_t *blk_size)
{
    ...
    ret = mpc_sie_get_block_size(&MPC_BRAM_DEV, blk_size);
...
static int32_t BRAM_MPC_GetCtrlConfig(uint32_t *ctrl_val)
{
    ...
    ret = mpc_sie_get_ctrl(&MPC_BRAM_DEV, ctrl_val);
...
static int32_t BRAM_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = mpc_sie_set_ctrl(&MPC_BRAM_DEV, ctrl);
                                        ...
                                        ARM_MPC_SEC_ATTR *attr)
{
    ...
    ret = mpc_sie_get_region_config(&MPC_BRAM_DEV, base, limit,
                                    (enum mpc_sie_sec_attr_t *)attr);
                                     ...
                                     ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = mpc_sie_config_region(&MPC_BRAM_DEV, base, limit,
                                (enum mpc_sie_sec_attr_t)attr);
...
static int32_t BRAM_MPC_EnableInterrupt(void)
{
    ...
    ret = mpc_sie_irq_enable(&MPC_BRAM_DEV);
...
{
    mpc_sie_irq_disable(&MPC_BRAM_DEV);
}


static void BRAM_MPC_ClearInterrupt(void)
{
    mpc_sie_clear_irq(&MPC_BRAM_DEV);
}

static uint32_t BRAM_MPC_InterruptState(void)
{
    return mpc_sie_irq_state(&MPC_BRAM_DEV);
}

static int32_t BRAM_MPC_LockDown(void)
{
    return mpc_sie_lock_down(&MPC_BRAM_DEV);
}
