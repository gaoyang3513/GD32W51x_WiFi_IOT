...
const struct wifi_mqtt_couple_t wifi_mqtt_couple[] = {
    {TOPIC_ZC_CHECKIN,         (void *)awss_enrollee_checkin},
    {TOPIC_ZC_ENROLLEE_REPLY,  (void *)awss_report_enrollee_reply},
    {TOPIC_ZC_CIPHER_REPLY,    (void *)awss_get_cipher_reply},
};
...
int wifi_mqtt_init(void *handle)
{
    ...
    for (i = 0; i < sizeof(wifi_mqtt_couple) / sizeof(wifi_mqtt_couple[0]); i ++) {
        ...
        ret = IOT_MQTT_Subscribe(handle, topic, (iotx_mqtt_qos_t)1, (iotx_mqtt_event_handle_func_fpt)wifi_mqtt_couple[i].cb, handle);
    }
