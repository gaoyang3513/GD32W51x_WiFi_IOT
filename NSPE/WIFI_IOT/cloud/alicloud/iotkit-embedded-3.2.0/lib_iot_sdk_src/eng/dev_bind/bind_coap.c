...
int bind_coap_init(void *handle, void *coap_handle)
{
    ...
    ret = bind_coap_register(bind_handle, BIND_TOPIC_DEVINFO_MCAST, (void *)bind_coap_get_token_mcast);
    if (ret < 0) {
    ...
    ret = bind_coap_register(bind_handle, buff, (void *)bind_coap_get_token_ucast);
    if (ret < 0) {
